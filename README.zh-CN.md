...
- dashboard 内置首次运行检查，会直接告诉用户当前缺的是权限、平板连接还是虚拟屏状态
- dashboard 还会给出“连接向导”和上下文恢复按钮，例如一键重建虚拟屏
- dashboard 现在还带验收测试面板，可记录通过/失败、填写备注并复制测试摘要
- 验收测试面板会自动推荐下一项，并支持上一项/下一项切换
- 验收测试入口现在放在 Mac dashboard 顶部区域，不需要去 Pad 端找
- 当触控权限或录屏权限有问题时，dashboard 还可以直接打开对应的 macOS 设置页
...
- `./start.sh` 一键启动
- `./start.sh stop / status / doctor` 这一套日常操作命令
...
`./start.sh start` 会自动做这些事：

- 查找或下载 `adb`
- 执行 `adb reverse tcp:9009 tcp:9009`
- 启动 host 服务
- 默认自动打开浏览器 dashboard
- host 启动后再做一次 USB / adb 链路自检
...
- 可做的事：
  ...
  - 关闭虚拟屏
  - 退出服务
  - 实时显示 USB / adb 状态，并一键修复 `adb reverse`
  - 刷新本机检查并显示下一步建议
  - 根据当前状态显示向导步骤和恢复动作
  - 记录验收测试结果并复制摘要
  - 自动推荐当前最值得先测的一项
